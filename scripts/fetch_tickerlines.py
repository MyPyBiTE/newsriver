...
- Reads  : ./newsriver/headlines.json       (single source of truth)
...
Selection:
  ...
  3) Pick top 3 unique items (max one per domain).
  4) Emit ticker schema:
     {
       "items":[{"text","display","url","flags":{...}},...],
       "generated_utc":"…Z"
     }
...
def to_ticker_wire(picked: List[Cand]) -> Dict[str, Any]:
    ...
    for c in picked:
        ...
        items.append({
            "text": c.item.title,
            "display": c.item.title,   # add display for front-end/YAML compatibility
            "url":  c.item.url,
            "flags": flags
        })
...
def main() -> int:
    ...
    ap.add_argument("--in",  dest="inp", default="./newsriver/headlines.json",
                    help="input enriched headlines JSON")
